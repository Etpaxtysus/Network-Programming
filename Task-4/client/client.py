import socket
import json
import os
import base64

def send_request(request_data, host='localhost', port=0000):
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    try:
        server_address = (host, port)
        print(f"\n[CLIENT LOG] Connecting to {host}:{port}...")
        sock.connect(server_address)
        sock.sendall(request_data)

        response_raw = b''
        while True:
            data = sock.recv(1024)
            if not data:
                break
            response_raw += data
        
        response_str = response_raw.decode('utf-8', errors='ignore')
        
        try:
            headers, body = response_str.split('\r\n\r\n', 1)
        except ValueError:
            headers = response_str
            body = ""

        print("\n--- SERVER RESPONSE ---")
        print(headers)
        if body.strip():
            print("-----------------------\nBody:")
            try:
                parsed_json = json.loads(body)
                print(json.dumps(parsed_json, indent=4))
            except (json.JSONDecodeError, TypeError):
                print(body.strip())
        
        print("-----------------------")
    except ConnectionRefusedError:
        print(f"\n[ERROR] Connection refused! Is the server running at {host}:{port}?")
    except Exception as e:
        print(f"\n[ERROR] An error occurred: {e}")
    finally:
        sock.close()
        

def list(host='localhost', port=0000):
    print("\n[INFO] Requesting file list...")
    request = b"GET /list HTTP/1.0\r\n\r\n\r\n"
    send_request(request, host, port)

def upload(filepath, host='localhost', port=0000):
    if not os.path.exists(filepath):
        print(f"\n[ERROR] File '{filepath}' not found locally.")
        return
        
    print(f"\n[INFO] Uploading '{filepath}'...") 

    with open(filepath, 'rb') as f:
        binary_content = f.read()
    
    base64_bytes = base64.b64encode(binary_content)
    base64_string = base64_bytes.decode('utf-8')

    body_content = base64_string.encode('utf-8')
    
    filename = os.path.basename(filepath)

    request_data = (
        f"POST /upload HTTP/1.0\r\n"
        f"X-Filename: {filename}\r\n"
        f"Content-Length: {len(body_content)}\r\n"
        f"Content-Transfer-Encoding: base64\r\n"
        f"\r\n"
    ).encode('utf-8') + body_content + b'\r\n\r\n\r\n'

    send_request(request_data, host, port)

def delete(filename, host='localhost', port=0000):
    print(f"\n[INFO] Deleting '{filename}'...")
    request = f"DELETE /delete/{filename} HTTP/1.0\r\n\r\n\r\n".encode('utf-8')
    send_request(request, host, port)
    
if __name__ == '__main__':
    # Change Port to 8885 for Thread Pool Server
    # Change Port to 8889 for Process Pool Server
    HOST='localhost'
    PORT = 8885

    print(f"\n===== STARTING CLIENT TEST (Port: {PORT}) =====")

    list(host=HOST, port=PORT)
    upload('../files/pokijan.jpg', host=HOST, port=PORT)
    list(host=HOST, port=PORT)
    delete('research_center.jpg', host=HOST, port=PORT)
    list(host=HOST, port=PORT)
    upload('keqing.png', host=HOST, port=PORT)

    print("\n===== CLIENT TEST COMPLETE =====")